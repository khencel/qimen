...
class AprilController extends Controller
{
    public function store($request,$last_chart){
        $minusDay = $request->total_days-20;
        $x = 61;
        ...
        for($i = 1;$i <= ($request->total_days - $minusDay);$i++){
            $my_chart = YearChart::create([
                ...
                'day_chart_id' => $last_chart > 120?$x++:$last_chart++,
                'month' => $request->month,
                'day' => $i,
                'year' => $request->year,
            ]);
            if($my_chart->month == 4 && $my_chart->day == 4){
                $month_id = $month_id+1;
            }
        }
    ...
    public function yangUpper($request,$month_id){
        ...
        $lower = DayChart::where('structure_type','Yang Structure')
                        ...
                        ->first();
        $x = 121;
        for($i = 21;$i <= $request->total_days;$i++){
            
            YearChart::create([
                'month_chart_id' => $month_id,
                'day_chart_id' => $lower->id > 180?$x++:$lower->id++,
                'month' => $request->month,
                'day' => $i,
                'year' => $request->year,
            ]);
