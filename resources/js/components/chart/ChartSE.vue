<template>
    <div>
        ...
        <div class="row mt-2">
            <div class="col-md-5">
                <div class="row justify-content-end pt-5 text-center bg-black position-relative">
                    <div class="text-white position-absolute m-1" style="z-index:1;top:0;left:0">
                        <h3>SE</h3>
                    </div>
                    <div class=" position-absolute mt-1" style=";width:100px;z-index:1;top:0;left:50%;margin-left:-27px;">
                        <div class="row justify-content-center">
                            <div  v-for="(item, index) in outsidePositions" :key="index" class="col-6">
                                <b-badge variant="success" class="p-2" v-text="item.value"></b-badge>
                                <br>
                                <input type="checkbox" v-model="se.seTop" :value="item.value">
                            </div>
                        </div>
                    </div>
                    <div class="position-absolute" style="height:100px;z-index:1;top:50%;left:0;margin-top:-50px;">
                        <div v-for="(item, index) in outsidePositions" :key="index" class="h-50 pt-2">
                            <b-badge variant="success" class="p-2" v-text="item.value"></b-badge>
                            <input type="checkbox" v-model="se.seLeft" :value="item.value">
                        </div>
                    </div>
                                        ...
                                        <div>
                                            <small>
                                                <select v-model="se.position_7">
                                                    <option :value="{id:null,fword:'',lword:''}" disabled hidden>position</option>
                                                    <option :value="{id:null,fword:'',lword:''}">None</option>
                                                    <option v-for="position in positions" :key="position.id" :value="{id:position.id,fword:position.first_word,lword:position.second_word}">
                                                        {{position.first_word}} {{position.second_word}} 
                                                    </option>
                                                </select>
            ...
            <div class="col-md-7 border">
                <div class=" pt-4 pb-4 pl-5 pr-5 position-relative" style="background-color:black">
                    ...
                    <div class=" position-absolute text-center" style="width:60px;z-index:1;top:0;left:15%">
                        <div class="row justify-content-center text-white" >
                            <div v-for="(item, index) in se.seTop" :key="index" class="col-6" v-show="item != ''">
                                <b-badge variant="success" v-text="item"></b-badge>
                            </div>
                        </div>
                    </div>

                    <div class=" position-absolute" style="height:50px;z-index:1;top:16%;left:0">
                        <div v-for="(item, index) in se.seLeft" :key="index">
                            <b-badge variant="success" v-text="item"></b-badge>
                        </div>
                    </div>
...
<script>
    export default {
        ...
        data(){
            return{
                ...
                se:new Form({
                    ...
                    position_7:{id:null,fword:'',lword:''},
                    seTop:[],
                    seLeft:[],
                    bird_2:false,
                }),
        ...
        methods:{
            ...
            chartUpdate(){
                this.se.put('/api/se/'+this.chart_id)
                .then(response => {
            ...
            async chartDetails(){
                this.se.get('/api/se/'+this.chart_id+'/'+this.type)
                .then(response => {
                    this.center = response.data.center.stem.value;
                    this.se.stem_top = {id:response.data.se.stem_top.id,name:response.data.se.stem_top.value};
                    this.se.star = {id:response.data.se.star.id,name:response.data.se.star.name,chinese:response.data.se.star.chinese};
                    this.se.deitie = {id:response.data.se.deitie.id,name:response.data.se.deitie.value,chinese:response.data.se.deitie.chinese};
                    this.se.stem_bottom = {id:response.data.se.stem_bottom.id,name:response.data.se.stem_bottom.value};
                    this.se.door = {id:response.data.se.door.id,name:response.data.se.door.name,chinese:response.data.se.door.chinese};
                    this.se.number = response.data.se.number;
                    response.data.se.seTop.split(",").forEach(element => {
                        this.se.seTop.push(element);
                    });

                    response.data.se.seLeft.split(",").forEach(element => {
                        this.se.seLeft.push(element);
                    });
            ...
            async fetchChartPosition(){
                ...
                .then(response => {
                        ...
                        this.se.position_7 = response.data.position.position_7 == null?this.se.position_7 = {id:null,fword:'',lword:''}:{id:response.data.position.position_7.id,fword:response.data.position.position_7.first_word,lword:response.data.position.position_7.second_word}; 
